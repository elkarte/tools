...
// A nice menu button
define('ELK_INTEGRATION_SETTINGS', serialize(array('integrate_menu_buttons' => 'fixp_menu_button',)));
...
{
	die('<b>Error:</b> Cannot find SSI - please verify you put this in the same place as ElkArtes\'s index.php.');
}
 ...
 * Copy the following function, change 'english' to the language you want
 * and translate it. ;D
 *
 */
...
/**
 * Load the language file, ok the language function :P
 *
 * @return mixed
 */
...
/**
 * Main processing function, shows the form and acts on requests
 *
 * @throws \Elk_Exception
 */
function fixp_main()
{
	global $txt, $boardurl, $context, $user_info;

	$db = database();

	// Use the standard ElkArte files
	loadLanguage('Admin');
	...
	$listOptions = array(
			...
			'install_date' => array(
				...
				'data' => array(
					'function' => function (&$data) {
						...
						return standardTime($data['time_' . (!empty($context['install']) ? 'installed' : 'removed')]);
					}
				)
			),
			'check' => array(
		...
		'form' => array(
			'href' => $boardurl . '/fixp.php?' . $context['session_var'] . '=' . $context['session_id'] . (!empty($context['install']) ? '' : ';uninstall'),
		),
		'additional_rows' => array(
			array(
				...
				'class' => 'submitbutton',
				'value' => '
				<a class="linkbutton" href="' . $boardurl . '/fixp.php' . (!empty($context['install']) ? '?uninstall' : '') . '">' . (!empty($context['install']) ? $txt['uninstall'] : $txt['install']) . '</a>
				<input type="submit" name="remove_packages" value="' . $txt['pack_button_' . (!empty($context['install']) ? 'remove' : 'install')] . '" />
				<input type="submit" name="remove_hooks" value="' . $txt['remove_hooks'] . '" />',
			),
		),
	);
...
/**
 * Fetch the packages that are logged in the system
 *
 * @return array
 */
function list_getPacks()
{
	...
	$request = $db->query('', '
		SELECT 
			id_install, name, version, time_installed, time_removed
...
function remove_hooks()
{
	...
	// Kill the cache - it needs redoing now.
